...
class PlayGame {
    ...
    func excute() {
        ...
        historyIntence.collectAnswer(answer: answer)
        while isPlaying {
            do {
                let userInput = try getUerIntput()
                historyIntence.collectUserInput(userInput: userInput)
                ...
                printMessage(resultOfDetermination: strikeAndBallCount)
                
                if strikeAndBallCount.0 == guesslength {
